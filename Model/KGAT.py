...
class KGAT(object):
    ...
    def _parse_args(self, data_config, pretrain_data, args):
        ...
        self.model_type += '_%s_%s_%s_l%d' % (
            args.adj_type, args.adj_uni_type, args.alg_type, self.n_layers)
    ...
    def _build_weights(self):
        ...
        if self.pretrain_data is None:
            all_weights['user_embed'] = tf.Variable(initializer(
                [self.n_users, self.emb_dim]), name='user_embed')
            all_weights['entity_embed'] = tf.Variable(initializer(
                [self.n_entities, self.emb_dim]), name='entity_embed')
        ...
        all_weights['relation_embed'] = tf.Variable(initializer(
            [self.n_relations, self.kge_dim]), name='relation_embed')
        all_weights['trans_W'] = tf.Variable(initializer(
            [self.n_relations, self.emb_dim, self.kge_dim]))
        self.weight_size_list = [self.emb_dim] + self.weight_size
        for k in range(self.n_layers):
            all_weights['W_gc_%d' % k] = tf.Variable(initializer(
                [self.weight_size_list[k], self.weight_size_list[k+1]]), name='W_gc_%d' % k)
            all_weights['b_gc_%d' % k] = tf.Variable(initializer(
                [1, self.weight_size_list[k+1]]), name='b_gc_%d' % k)
            all_weights['W_bi_%d' % k] = tf.Variable(initializer(
                [self.weight_size_list[k], self.weight_size_list[k + 1]]), name='W_bi_%d' % k)
            all_weights['b_bi_%d' % k] = tf.Variable(initializer(
                [1, self.weight_size_list[k + 1]]), name='b_bi_%d' % k)
            all_weights['W_mlp_%d' % k] = tf.Variable(initializer(
                [2 * self.weight_size_list[k], self.weight_size_list[k+1]]), name='W_mlp_%d' % k)
            all_weights['b_mlp_%d' % k] = tf.Variable(initializer(
                [1, self.weight_size_list[k+1]]), name='b_mlp_%d' % k)
        return all_weights
    ...
    def _build_model_phase_I(self):
        if self.alg_type in ['bi', 'kgat']:
            ...
            print(self.ua_embeddings)
            print("\n\n")
            print(self.ea_embeddings)
    ...
    def _get_kg_inference(self, h, r, pos_t, neg_t):
        ...
        # Debug prints to check indices and embedding dimensions
        print(f"Shape of embeddings: {embeddings.shape}")
        print(
            f"Max value in h: {tf.reduce_max(h)}, Min value: {tf.reduce_min(h)}")
        print(
            f"Max value in pos_t: {tf.reduce_max(pos_t)}, Min value: {tf.reduce_min(pos_t)}")
        print(
            f"Max value in neg_t: {tf.reduce_max(neg_t)}, Min value: {tf.reduce_min(neg_t)}")
    ...
    def _build_loss_phase_I(self):
        ...
        regularizer = tf.nn.l2_loss(
            self.u_e) + tf.nn.l2_loss(self.pos_i_e) + tf.nn.l2_loss(self.neg_i_e)
    ...
    def _build_loss_phase_II(self):
        ...
        kg_loss = tf.reduce_mean(
            tf.nn.softplus(-(neg_kg_score - pos_kg_score)))
        ...
        self.opt2 = tf.train.AdamOptimizer(
            learning_rate=self.lr).minimize(self.loss2)
    ...
    def _create_attentive_A_out(self):
        ...
        A = tf.sparse.softmax(tf.SparseTensor(
            indices, self.A_values, self.A_in.shape))
        return A
    ...
    def evaluate(self, sess, test_users, test_items, K):
        mrr = self.calculate_mrr(sess, test_users, test_items, K)
        return mrr
    
    def calculate_mrr(self, sess, test_users, test_items, K):
        feed_dict = {
            self.users: test_users,
            self.pos_items: test_items,
            self.node_dropout: [0.0] * len(self.weight_size),
            self.mess_dropout: [0.0] * len(self.weight_size)
        }
        predictions = sess.run(self.batch_predictions, feed_dict=feed_dict)
        mrr = self.compute_mrr(predictions, K)
        return mrr

    def compute_mrr(self, predictions, K):
        """
        Compute MRR for the given predictions.
        :param predictions: Predicted scores (num_users, num_items).
        :param K: Number of top items to consider for evaluation.
        ...
        for i in range(num_users):
            ...
            sorted_items = np.argsort(-user_predictions)[:K]
            rank = np.where(sorted_items == 0)[0][0] + 1  # Assumes that test item is at position 0
            mrr += 1.0 / rank
        mrr /= num_users
        return mrr