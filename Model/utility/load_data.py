...
class Data(object):
    ...
    def _statistic_ratings(self):
        self.n_users = max(max(self.train_data[:, 0]), max(
            self.test_data[:, 0])) + 1
        self.n_items = max(max(self.train_data[:, 1]), max(
            self.test_data[:, 1])) + 1
    ...
    def _print_data_info(self):
        ...
        print('[n_entities, n_relations, n_triples]=[%d, %d, %d]' %
              (self.n_entities, self.n_relations, self.n_triples))
        print('[batch_size, batch_size_kg]=[%d, %d]' %
        ...
        def sample_neg_items_for_u(u, num):
            ...
            while True:
                ...
                neg_i_id = np.random.randint(
                    low=0, high=self.n_items, size=1)[0]
